...
class InputData:
    ...
    def __init__(self, data_path: str):
        ...
        halton_data = pd.read_csv(os.path.join(data_path, 'input', 'halton_sample.csv'))
        ...
        self.halton_x = torch.tensor(halton_data.iloc[:, :-1].values, dtype=torch.float32)
        self.halton_y = torch.tensor(halton_data.iloc[:, -1].values.reshape(-1, 1), dtype=torch.float32)
    ...
    def get_training_and_test_data(self, sampling_method: SamplingMethod, training_set_size: int) -> TrainingData:
        ...
        elif sampling_method == SamplingMethod.HALTON:
            return TrainingData(self.halton_x[:training_set_size], self.halton_y[:training_set_size], self.mc_x, self.mc_y)
        else:
